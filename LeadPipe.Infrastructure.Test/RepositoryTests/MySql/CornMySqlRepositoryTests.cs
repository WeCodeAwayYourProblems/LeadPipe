...
{
    private static CornMySqlRepository CreateRepo(out MySqlSchema3Context context)
    {
        context = DbContextTestFactory.CreateTestContext<MySqlSchema3Context>(
            nameof(CornMySqlRepositoryTests));
    ...
    {
        CornMySqlRepository repo = CreateRepo(out var ctx);
    ...
    {
        CornMySqlRepository repo = CreateRepo(out _);
